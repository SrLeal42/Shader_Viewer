...
// Função matemática para calcular a distância de um ponto até um segmento de linha (3D).
// Retorna a distância e preenche 'h' (que vai de 0.0 na cauda a 1.0 na cabeça)
...
// Função para gerar um vetor 3D aleatório baseado numa semente
vec3 hash3(float n) {
        ...
        if (t < launchEnd) {
            ...
            if (distToCenter < 0.8 && fadeOut > 0.0) {
                ...
                for (int j = 0; j < NUM_PARTICLES; j++) {
                    ...
                    // Direção aleatória de cada partícula (distribuição esférica)
